...
1. **Phases 1-2 (years 4-6):** Earth launches the lunar testbed. The Moon validates extraction, smelting, and assembly technologies in vacuum and low gravity conditions.

2. **Phases 3-4 (years 6-10):** Knowledge from the Moon is applied on Mercury. The Ground Zero Factory copies itself exponentially, reaching ~1650 factories (~1500 F-M + ~150 F-R). Mirrors are produced and sent to the Swarm.

3. **Phase 5 (years 7+):** The Dyson Swarm reflects light to lunar LSP stations. LSP converts light to microwaves and transmits to Earth. Closed cycle: energy -> production -> more energy.
...
| **Specialization** | Mass mirror production | High-tech electronics |
| **Strategy** | Exponential growth | Direct delivery + assembly |
...
**The Moon** is the gateway to space. A 1.3-second communication delay allows Earth operators to control the process in near real-time. Ideal for technology validation and complex electronics production.
...
> - **Throughput:** 600 t regolith/day -> **42 t Al** + **18 t Fe** -> **~407 mirrors/day** (F-M)
...
| Internal dome environment | Inert gas (nitrogen) 0.1 atm |
...
**Total area:** ~1500 m^2 (~50x30 m) - under 0.1 atm nitrogen pressure.
...
| Nitrogen (N2) | Absent | Dome atmosphere - import from Earth |
...
> **Details:** [Production Energy](energy.qmd)

Solar Boost (9-14 kW/m^2), Power Towers with GaAs cells, cryogenic power lines, silicon panel production (aluminothermy) and silicate domes.
...
Gas environment already present here (nitrogen 0.1 atm).
...
LEGEND:
  [*]  = charging station (Crabs, Centaurs outside)
  *=== = cable point + cable (Moles)
  [*][*][*] = station cluster
```
...
| NaS batteries (Gen-1) | 15 | 750 kg | Reserve |
...
| Category | Mass | Note |
| :--- | :--- | :--- |
| Gases (Ar, N2 compressed) | 0.5 t | |
| Dies + end mills (W/WC) | 0.1 t | |
| Copper mold (spare) | 0.3 t | |
| MRE anodes (reserve) | 0.2 t | |
| Oils, lubricants | 0.1 t | |
| Beta-Al2O3 electrolytes | 0.3 t | For first 50 batteries |
| **Total consumables** | **~1.5 t** | |
...
| Consumables (1 year) | 1.5 t |
| Vitamins (electronics) | 1.0 t |
| **TOTAL first expedition** | **~62 t** |

---

### Assembly Optimization: "Blitz Assembly"

*7 days from landing to first Gen-2 robot.*

Gen-1 robots are light but fragile (titanium, carbon fiber). Lifespan is limited. The longer assembly takes, the higher the risk of breakdown before factory startup.

**Solution:** Compress assembly to 7 days through pre-assembly and parallelism.

#### Critical Path: 7 Days

| Day | Main Work | Result |
| :--- | :--- | :--- |
| 1 | Unloading + mirror deployment | Modules in position |
| 2 | Mirrors + MRE on site | MRE ready to operate |
| 3 | Dome inflation + power supply | **Power available!** |
| 4 | CCM and rolling mill into dome | Metal flowing |
| 5 | Finishing module + commissioning | Line ready |
| 6 | First smelt -> casting -> rolling | Material for WAAM |
| 7 | WAAM + CNC + assembly | **First Gen-2** |
...
**Details:** [Energy Calculations](energy.qmd#mirror-disposal)
...
| Parameter | Value |
|----------|-------|
| Requirement for 1000 factories | ~18,000 tons (over 10 years) |
| World production | ~85,000 t/year |
| Share of world production | **~2%/year** |
...
| Vitamins | Years 1-10 | ~30 kg/factory/year |

**Vitamins** are what Mercury doesn't have: tungsten (dies), iridium (anodes), rare earths (magnets), carbon (lubricants).
